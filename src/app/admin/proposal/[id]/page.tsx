...
import { getProposalById, reviewProposal } from '@/lib/actions/proposal';
...
interface TeamMember {
  ...
  user: {
    ...
    username: string;
    email: string;
    dosen?: { nama: string; nidn?: string; fakultas?: string; prodi?: string }[];
    mahasiswa?: { nama: string; nim?: string; fakultas?: string; prodi?: string }[];
  };
}
...
export default function AdminProposalDetailPage() {
  ...
  const loadProposal = async () => {
    ...
    if (data) {
      ...
      setReviewData({
        status: data.status_proposal === 'submitted' ? 'review' : data.status_proposal,
        catatan: data.catatan_evaluasi || '',
        anggaranDisetujui: data.anggaran_disetujui || data.anggaran_diajukan || 0,
      });
    }
    if (error) {
      setError(error);
  ...
  const getMemberInfo = (member: TeamMember) => {
    const dosen = member.user.dosen?.[0];
    const mahasiswa = member.user.mahasiswa?.[0];
    if (dosen) {
      return { type: 'Dosen', id: dosen.nidn || '-', fakultas: dosen.fakultas, prodi: dosen.prodi };
    }
    if (mahasiswa) {
      return { type: 'Mahasiswa', id: mahasiswa.nim || '-', fakultas: mahasiswa.fakultas, prodi: mahasiswa.prodi };
    }
    return { type: 'User', id: '-', fakultas: null, prodi: null };
  };
              ...
              {proposal.ketua ? (
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">{proposal.ketua.nama}</p>
                      <p className="text-sm text-gray-600">NIDN: {proposal.ketua.nidn}</p>
                    </div>
                  </div>
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center gap-2 text-gray-600">
                      <Building className="h-4 w-4" />
                      <span>{proposal.ketua.fakultas || '-'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-600">
                      <GraduationCap className="h-4 w-4" />
                      <span>{proposal.ketua.prodi || '-'}</span>
                    </div>
                  </div>
                </div>
              ) : (
                <p className="text-sm text-gray-500 text-center py-4">Data ketua tidak tersedia</p>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-3">
              {proposal.dokumen_proposal_url && (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-blue-700">Dokumen Utama Proposal (PDF)</p>
                    <p className="text-xs text-blue-500 truncate">{proposal.dokumen_proposal_url}</p>
                  </div>
              ...
              {proposal.link_drive && (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-green-700">Link Google Drive</p>
                    <p className="text-xs text-green-500 truncate">{proposal.link_drive}</p>
                  </div>
                  <ExternalLink className="h-5 w-5 text-green-600" />
                </a>
              )}

              {proposal.dokumen && proposal.dokumen.length > 0 && (
                <>
                  {proposal.dokumen.map((doc) => (
                    <a 
                      key={doc.id}
                      href={doc.file_url} 
                      target="_blank" 
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-700">{doc.nama_dokumen}</p>
                        {doc.jenis_dokumen && (
                          <p className="text-xs text-gray-500">{doc.jenis_dokumen}</p>
                        )}
                      </div>
              ...
              {!proposal.dokumen_proposal_url && !proposal.link_drive && (!proposal.dokumen || proposal.dokumen.length === 0) && (
                    ...
                    .map((member) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-500">{info.type} • {info.id}</p>
                              </div>
                            </div>
                          ...
                          {(info.fakultas || info.prodi) && (
                            <div className="text-xs text-gray-500 ml-13 pl-13">
                              {info.fakultas} {info.prodi && `• ${info.prodi}`}
                            </div>
                          )}
                        </div>
                      );
